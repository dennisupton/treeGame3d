...
func fadeBus(bus_name: String, target_db: float, duration := 2.0) -> Tween:
	...
	if idx == -1:
		...
		return null
	# cancel any fade already running on this bus so they don't fight
	var running = _busFades.get(idx)
	if running and running.is_valid():
		running.kill()
	AudioServer.set_bus_mute(idx, false)   # make sure it can be heard
...
## Fade in to full volume (0 dB). `from_silence` snaps to silent first
## so it always fades up from nothing; set false to fade up from wherever it is.
